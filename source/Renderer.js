...
export class Renderer {
    ...
    print_results(max_count = 30, recalculate=true) {
        let twins_list = this.udb.sortedList(max_count, 0, recalculate);
    ...
    displayImages(opts = {}) {
        ...
        if (images == null) {
            if (opts.ids) {
                ...
                images = this.idb.sortedListExcluding(this.c.getHidden(), null, 0, opts.recalculate);
                opts.mode = "excluding"
            } else if (opts.all || opts.mode == "all") {
                if (opts.recalculate) { this.udb.calculateScores() }
                images = this.idb.sortedList(null, 0, opts.recalculate)
                opts.mode = "all"
            } else {
